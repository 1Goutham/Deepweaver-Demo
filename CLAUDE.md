...
- Contact form: `/api/contact` validates server-side, rate-limits per address, drops honeypot submissions and emails the enquiry to `CONTACT_TO_EMAIL` (default: the site email) with reply-to set to the enquirer. Needs `RESEND_API_KEY` and a verified `CONTACT_FROM_EMAIL`; see `.env.example`. Without the key the route returns 503 and the form shows an error with a mail fallback, never a silent failure.
